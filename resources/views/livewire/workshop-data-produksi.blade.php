        ...
        @elseif ($errorMessage)
            ...
            <div class="p-6 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900 rounded-3xl flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-rose-600 dark:text-rose-500 mt-1">Sistem gagal berkomunikasi dengan API Produksi: {{ $errorMessage }}</p>
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-6 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-6">
                <div class="flex items-center gap-4 flex-1">
                    ...
                    <div>
                        <h2 class="font-black text-xl md:text-2xl text-slate-800 dark:text-white tracking-tight">
                            Data Produksi <span class="text-slate-400 font-medium">(Sedang Diproduksi)</span>
                        </h2>
                        <p class="text-[9px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest mt-1">
                            DAFTAR SPK YANG SEDANG BERADA DI TAHAP PRODUKSI BESERTA PEMANTAUAN ESTIMASI SELESAI
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 items-center w-full lg:w-auto">
                    ...
                    <div class="w-full sm:w-[160px] bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl p-4 text-center">
                        <span class="text-[9px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider block">TOTAL DI PRODUKSI</span>
                        <span class="text-3xl font-black text-emerald-600 dark:text-emerald-400 block my-1 font-mono">
                            {{ number_format($summary['total_items_in_production'] ?? 0) }}
                        </span>
                        <span class="text-[8px] font-semibold text-slate-400 uppercase block">SPK dalam produksi</span>
                    </div>
                ...
                <div class="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap p-1 bg-slate-100 dark:bg-gray-850 rounded-2xl self-start gap-1 sm:gap-0">
                        <button wire:click="setStatusFilter('')" 
                            class="px-4 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-wider {{ $selectedStatus === '' ? 'bg-white dark:bg-gray-800 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white' }}">
                            Semua Status
                        </button>
                        <button wire:click="setStatusFilter('on_track')" 
                            class="px-4 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-wider {{ $selectedStatus === 'on_track' ? 'bg-white dark:bg-gray-800 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white' }}">
                            On Track
                        </button>
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <!-- PDF Download Link -->
                        <a href="{{ route('workshop-data-produksi.export-pdf', ['searchTerm' => $searchTerm, 'selectedStatus' => $selectedStatus, 'selectedService' => $selectedService, 'selectedCategory' => $selectedCategory, 'selectedEstimation' => $selectedEstimation, 'estimationStartDate' => $estimationStartDate, 'estimationEndDate' => $estimationEndDate]) }}" 
                            target="_blank"
                            class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-2xl text-center shadow-md shadow-emerald-500/10 transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
                            <span>📄</span>
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-6 shadow-sm">
                <div class="flex items-center justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">DAFTAR SEPATU DI BAGIAN PRODUKSI DENGAN ESTIMASI DAN STATUS SLA</p>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-gray-900/50 border-b border-slate-100 dark:border-gray-800 uppercase text-[9px] font-black text-slate-400 dark:text-gray-500 tracking-wider">
                                ...
                                <th class="px-5 py-4">WAKTU MASUK PRODUKSI</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-gray-800 uppercase font-bold text-slate-700 dark:text-gray-300 text-[10px]">
                            @forelse($paginatedItems as $item)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-gray-800/20 transition-colors">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex flex-col">
                                            <span class="font-mono text-slate-600 dark:text-gray-400">{{ $item['entered_production_at_formatted'] ?: '-' }}</span>
                                            <span class="text-[8px] font-normal text-emerald-500 uppercase mt-0.5 tracking-tight">
                                                {{ $item['days_in_production'] ?? 0 }} Hari di Produksi
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-center">
                                        @if($item['has_estimation'])
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[8px] font-black tracking-widest bg-emerald-55 dark:bg-emerald-950/20 text-emerald-600 border border-emerald-200/50">
                                    ...
                                    <td class="px-5 py-4 text-center">
                                        @if($item['has_estimation'])
                                            ...
                                            @endphp
                                            @if($daysDiff < 0)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest bg-rose-50 text-rose-600 border border-rose-200/50 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900/30">
                                                    KELEWAT {{ abs($daysDiff) }} HARI
                                                </span>
                                            @elseif($daysDiff == 0)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest bg-amber-50 text-amber-600 border border-amber-200/50 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/30">
                                                    HARI INI
                                                </span>
                                            @else
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest bg-emerald-50 text-emerald-600 border border-emerald-200/50 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/30">
                                                    SISA {{ $daysDiff }} HARI
                                                </span>
                                            @endif
                                        @else
                                ...
                                <tr>
                                    <td colspan="8" class="px-8 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3 opacity-30">
                                            ...
                                            <p class="text-xs">Tidak ada data produksi ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            @for($i = $startPage; $i <= $endPage; $i++)
                                <button wire:click="setPage({{ $i }})" 
                                    class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all {{ $i === $currentPage ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/10' : 'bg-slate-50 hover:bg-slate-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300 border border-slate-200/50 dark:border-gray-800' }}">
                                    {{ $i }}
                                </button>
                            @endfor
