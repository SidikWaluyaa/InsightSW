        ...
        @elseif ($errorMessage)
            ...
            <div class="p-6 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900 rounded-3xl flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-rose-600 dark:text-rose-500 mt-1">Sistem gagal berkomunikasi dengan API Piutang After: {{ $errorMessage }}</p>
                ...
                <div class="flex items-center gap-4 flex-1">
                    ...
                    <div>
                        <h2 class="font-black text-xl md:text-2xl text-slate-800 dark:text-white tracking-tight">
                            Piutang After <span class="text-xs px-2.5 py-1 bg-emerald-500/10 text-emerald-500 rounded-full border border-emerald-500/20 ml-2 uppercase font-black tracking-widest animate-pulse">Live API</span>
                        </h2>
                        <p class="text-[9px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest mt-1">
                            DAFTAR INVOICE YANG BELUM LUNAS SETELAH PROSES PENGERJAAN SELESAI — REALTIME SYNC
                        </p>
                    </div>
                </div>
                
                <div class="flex flex-col sm:flex-row gap-4 items-center w-full lg:w-auto">
                    <div class="w-full sm:w-[200px] bg-indigo-50/50 dark:bg-indigo-950/10 border border-indigo-100 dark:border-indigo-900/30 rounded-2xl p-4 text-center">
                        <span class="text-[9px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-wider block">TOTAL INVOICE</span>
                        <span class="text-4xl font-black text-indigo-600 dark:text-indigo-400 block my-1 font-mono">
                            {{ number_format($totalPiutangCount) }}
                        </span>
                        <span class="text-[8px] font-semibold text-slate-400 uppercase block">Invoice After Belum Lunas</span>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div class="relative">
                            <select wire:model.live="selectedStatus" 
                                class="w-full sm:w-48 px-4 py-3 bg-slate-50 dark:bg-gray-800/40 border border-slate-200/50 dark:border-gray-800 rounded-2xl text-xs font-bold text-slate-700 dark:text-gray-300 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-all cursor-pointer shadow-sm">
                        ...
                        {{-- PDF Download Link --}}
                        <a href="{{ route('finance-piutang-after.export-pdf', ['searchTerm' => $searchTerm, 'selectedStatus' => $selectedStatus]) }}" 
                            target="_blank"
                            class="px-5 py-3 bg-indigo-500 hover:bg-indigo-600 text-white font-black text-xs rounded-2xl text-center shadow-md shadow-indigo-500/10 transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
                            <span>📄</span>
                            <span>CETAK PDF</span>
                        </a>
                ...
                <div class="flex items-center justify-between gap-4 mb-6">
                    <div>
                        <h4 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider">
                            DAFTAR PIUTANG AFTER (OUTSTANDING)
                        </h4>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">INVOICE SETELAH PENGERJAAN YANG BELUM DILUNASI BESERTA DETAIL SEPATU DAN LAYANAN</p>
                    </div>
                            ...
                            @forelse($paginatedItems as $item)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-gray-800/20 transition-colors align-top">
                                    ...
                                    <td class="px-5 py-4 text-center">
                                        ...
                                        @elseif(str_contains($statusText, 'DP') || str_contains($statusText, 'CICIL') || str_contains($statusText, 'SEBAGIAN'))
                                ...
                                <tr>
                                    <td colspan="7" class="px-8 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3 opacity-30">
                                            ...
                                            <p class="text-xs">Tidak ada data piutang after ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @for($i = $start; $i <= $end; $i++)
                                <button wire:click="setPage({{ $i }})" 
                                    class="w-9 h-9 rounded-xl text-[10px] font-black transition-all {{ $i === $currentPage ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-300 hover:bg-slate-200 dark:hover:bg-gray-700' }}">
                                    {{ $i }}
                                </button>
                            @endfor
